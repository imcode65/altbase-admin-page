...
const Paginator: FC<IPaginator> = ({ totalPages=1, currentPage=1, btnClickHandler=()=>{} }) => {
    ...
    return (<div className='cursor-pointer flex'>
        <button onClick={() => btnClickHandler(1)} className='w-8 h-8 border rounded-l-md flex items-center justify-center'>
            <IconDoubleLeft className='w-3 h-3' />
            <IconDoubleLeft className='w-3 h-3' />
        </button>
        <button onClick={() => btnClickHandler(currentPage - 1)} className='w-8 h-8 border-t border-b border-r flex items-center justify-center'>
            <IconDoubleLeft className='w-3 h-3' />
        </button>
        { pages.map((page,id) => 
        ...
        <button onClick={() => btnClickHandler(totalPages)} className='w-8 h-8 border rounded-l-md flex items-center justify-center'>
            <IconDoubleRight className='w-3 h-3' />
            <IconDoubleRight className='w-3 h-3' />
        </button>
    </div>)
